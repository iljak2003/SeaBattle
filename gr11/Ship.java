...
public class Ship {
    ...
    public static ArrayList<String> sizeOfFleet(int boardSize) {
        ...
        int maxCells = (int) Math.floor(boardSize * boardSize  * 0.18);
        while (regularCells(A, B, D, C, P) > maxCells){
            if (A > 0){
                A--;
            }else if (B > 0){
                B--;
    ...
    private static int regularCells(int A, int B, int D, int C, int P){
        return A * 5 + B * 4 + D * 3 + C * 2 + P;
    }
